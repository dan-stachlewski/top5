        ...
        <fieldset>
            <table>
                <tr>
                    ...
                    <td> <?php if ($page['mode'] === USER_PAGE_MODE_REGISTER_NEW): ?>
                ...
                <tr>
                    ...
                    <?php if ($page['mode'] === USER_PAGE_MODE_REGISTER_NEW): ?>
                        <td>*<input type="password" required=""  placeholder="********"  name="password" value="<?= $user['password']; ?>" maxlength="40" /></td>
                    <?php else: ?>
                        <td><a href=".?action=show_password_change_form">change password</a></td>
                    <?php endif; ?>
                </tr>

                <tr>
                    <td> <input type="hidden" name="action" value="<?= $page['action'] ?>" />
                        <button  type="button" onclick="window.location='<?= $page['cancel_link'] ?>';return false;">&rarr; Cancel</button></td>
